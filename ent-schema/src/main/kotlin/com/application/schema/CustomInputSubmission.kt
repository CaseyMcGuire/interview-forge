...
class CustomInputSubmission : EntSchema("custom_input_submissions", clientName = "customInputSubmissions") {
  ...
  val byStatusAndCreatedAt = index("idx_custom_input_submissions_status_created_at", status, timestamps.createdAt)

  val byUser = index("idx_custom_input_submissions_user", user.fk)
  val byProblemLanguage = index("idx_custom_input_submissions_problem_language", problemLanguage.fk)
  val byExpiresAt = index("idx_custom_input_submissions_expires_at", expiresAt)
}
