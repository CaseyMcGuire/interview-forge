...
class ProblemLanguage : EntSchema("problem_languages", clientName = "problemLanguages") {
  ...
  val problem by belongsTo<Problem>("problem").field(problemId)
    .inverse(Problem::languageConfigurations).onDelete(OnDelete.RESTRICT)
  ...
  val language by belongsTo<Language>("language").field(languageId)
    .inverse(Language::problemConfigurations).onDelete(OnDelete.RESTRICT)
  ...
  val byProblemAndLanguage = index("uq_problem_languages_problem_language", problemId, languageId).unique()
