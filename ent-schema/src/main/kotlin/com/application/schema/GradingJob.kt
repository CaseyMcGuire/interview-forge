...
class GradingJob : EntSchema("grading_jobs", clientName = "gradingJobs") {
  ...
  val problemSubmission by belongsTo<ProblemSubmission>("problem_submission_id")
    ...
    .comment("Exactly one of problemSubmission and customInputSubmission identifies the result destination.")
  ...
  val problemLanguage by belongsTo<ProblemLanguage>("problem_language_id")
    ...
    .comment("Selects the language and current judge configuration; the runtime stays in application config.")
  ...
  val byStatusAndCreatedAt by index("idx_grading_jobs_status_created_at", status, createdAt)
  val byProblemLanguage by index("idx_grading_jobs_problem_language", problemLanguage.fk)
}
