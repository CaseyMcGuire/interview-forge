...
class SubmissionTestResult : EntSchema("submission_test_results", clientName = "submissionTestResults") {
  ...
  val submission by belongsTo<Submission>("submission").field(submissionId)
    .inverse(Submission::testResults).onDelete(OnDelete.RESTRICT)
  ...
  val testCase by belongsTo<TestCase>("test_case").field(testCaseId).nullable().onDelete(OnDelete.SET_NULL)
  ...
  val expectedOutputJson by json<JsonElement>("expected_output_json").nullable().immutable().sensitive()
  ...
  val bySubmissionAndPosition = index("uq_submission_test_results_submission_position", submissionId, position).unique()
