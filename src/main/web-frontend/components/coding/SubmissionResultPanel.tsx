...
import type {
  SubmissionResultPanel_submission$data,
  SubmissionResultPanel_submission$key,
  SubmissionVerdict
} from "__generated__/SubmissionResultPanel_submission.graphql";
import SubmissionFailedExample from "./SubmissionFailedExample";
import Control from "./WorkspaceControl";
import Icon from "./WorkspaceIcon";

type Props = {
  submission: SubmissionResultPanel_submission$key | null;
  isLoading?: boolean;
  error?: string | null;
  onRetry?: () => void;
};

const verdictSummaries: Partial<Record<SubmissionVerdict, {label: string; message: string}>> = {
  ACCEPTED: {
    label: "Accepted",
...
function describeSubmission(
  submission: SubmissionResultPanel_submission$data | null | undefined,
  isLoading: boolean,
  hasError: boolean
) {
  ...
  if (submission.status === "FINISHED") {
    ...
    return {
      ...
      message: submission.publicErrorMessage ?? summary?.message ?? "No verdict is available for this submission."
    };
  }
...
export default function SubmissionResultPanel(props: Props) {
  ...
  return (
    <section sx={styles.panel} aria-labelledby={headingId}>
      ...
      <div sx={styles.body}>
          ...
          <dl sx={styles.statistics}>
            {submission.totalCases > 0 && (
              <div sx={styles.statistic}>
                ...
                <dd sx={styles.statisticValue}>{submission.passedCases} / {submission.totalCases} cases</dd>
              </div>
            )}
        ...
        {isFinished && submission.failedExample && (
          <SubmissionFailedExample example={submission.failedExample} />
        )}
      </div>
    </section>
  );
