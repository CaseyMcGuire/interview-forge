...
@Import(ProblemSubmissionIntegrationTest.ProblemSubmissionTestConfiguration::class)
class ProblemSubmissionIntegrationTest {
  ...
  fun setUp() {
    entClient.withTransaction { tx ->
      tx.problemSubmissionFailures.deleteMany(fixtureContext).getOrThrow()
      tx.problemSubmissions.deleteMany(fixtureContext).getOrThrow()
    }.getOrThrow()
    ...
    problem = entClient.problems.create {
      slug = "problem-submission-${UUID.randomUUID()}"
      title = "Problem submission fixture"
  ...
  fun `official submissions retain source and atomically enqueue selected test inputs`() {
    ...
    val problemSubmission = result["problemSubmission"]
    assertEquals("QUEUED", problemSubmission["status"].asString())
    assertEquals("PENDING", problemSubmission["verdict"].asString())
    assertTrue(problemSubmission["startedAt"].isNull)
    assertTrue(problemSubmission["finishedAt"].isNull)
    assertEquals(2, problemSubmission["totalCases"].asInt())
    assertEquals(0, problemSubmission["passedCases"].asInt())
    assertFalse(problemSubmission.has("testResults"))
    assertFalse(problemSubmission.toString().contains("secret"))
    assertFalse(problemSubmission.toString().contains("private driver"))

    val stored = storedProblemSubmissions().single()
    assertEquals(userId, stored.userId)
    assertEquals(configurationId, stored.problemLanguageId)
    assertEquals("  solution source\n", stored.sourceCode)
    assertEquals(ProblemSubmissionVerdict.PENDING, stored.verdict)
    val job = entClient.gradingJobs.indexes.problemSubmissionId(stored.id).query()
    ...
    assertEquals(problemSubmission, poll(problemSubmission["id"].asString()))
  }
  ...
  fun `the API has no example run mutation or custom case input`() {
    val runResponse = graphql(
      "mutation { runProblemSubmission(input: {problemLanguageId: \"unused\", sourceCode: \"solution\"}) { __typename } }",
      emptyMap(),
      session,
      allowErrors = true,
    )
    ...
    assertTrue(storedProblemSubmissions().isEmpty())
    assertTrue(storedCases().isEmpty())
  }
  ...
  fun `invalid source returns field errors without creating a submission`() {
    ...
    for (sourceCode in listOf(" \n", "x".repeat(50_001))) {
      ...
      assertEquals("ProblemSubmissionValidationFailure", result["__typename"].asString(), result.toString())
  ...
  fun `source validation also applies to entity creation outside the service`() {
    for (sourceCode in listOf(" \n", "x".repeat(50_001))) {
      val exception = assertThrows(EntValidationException::class.java) {
        createProblemSubmissionDirectly(sourceCode)
      }
    ...
    assertEquals(sourceCode, createProblemSubmissionDirectly(sourceCode).sourceCode)
  }

  @Test
  fun `only the owner can poll official submissions even after archival`() {
    createCase(0, TestCaseVisibility.HIDDEN, "1")
    val problemSubmission = submitSolution()["problemSubmission"]
    val id = problemSubmission["id"].asString()
    ...
    for (invalidId in listOf(
      ...
      globalIdUtil.toGlobalId(GraphqlProblemSubmission::class, Long.MAX_VALUE),
    )) {
    ...
    assertEquals(problemSubmission, poll(id))
  }

  @Test
  fun `polling reflects persisted official execution progress and verdicts`() {
    createCase(0, TestCaseVisibility.HIDDEN, "1")
    val publicId = submitSolution()["problemSubmission"]["id"].asString()
    val stored = storedProblemSubmissions().single()
    val started = Instant.now()

    entClient.problemSubmissions.update(stored.id) {
  ...
  fun `authentication missing targets and unavailable execution return expected failures`() {
    ...
    for (id in listOf(
      "bad!",
      globalIdUtil.toGlobalId(GraphqlProblemSubmission::class, configurationId),
  ...
  fun `concurrent requests cannot exceed the per-user submission limit`() {
    ...
    try {
      ...
      assertSingleAcceptedProblemSubmission(outcomes)
      assertEquals(2, storedProblemSubmissions().size)
      assertTrue(storedCases().isEmpty())
  ...
  fun `the last global queue slot can be accepted only once across different users`() {
    ...
    try {
      ...
      val outcomes = requests.map { it.get(30, TimeUnit.SECONDS) }
      assertSingleAcceptedProblemSubmission(outcomes)
      assertEquals(4, storedProblemSubmissions().size)
  ...
  fun `privacy permits official test reads only for execution and denies ordinary lifecycle writes`() {
    ...
    assertSuccess(submitSolution())
    val problemSubmission = storedProblemSubmissions().single()
    ...
    try {
      ...
      assertEquals(problemSubmission.id, entClient.problemSubmissions.findById(owner, problemSubmission.id).getOrThrow()!!.id)

      for (viewer in listOf(Viewer.Anonymous, Viewer.User(otherId))) {
        assertThrows(EntPrivacyDeniedException::class.java) {
          entClient.problemSubmissions.findById(ViewerContext(viewer), problemSubmission.id).getOrThrow()
        }
      }

      assertThrows(EntMutationPrivacyDeniedException::class.java) {
        entClient.problemSubmissions.update(problemSubmission.id) {
          status = ProblemSubmissionStatus.FINISHED
        }.save(owner).getOrThrow()
      }
      assertThrows(EntMutationPrivacyDeniedException::class.java) {
        entClient.problemSubmissions.deleteById(owner, problemSubmission.id).getOrThrow()
      }
      assertThrows(EntMutationPrivacyDeniedException::class.java) {
        entClient.problemSubmissions.create {
          userId = this@ProblemSubmissionIntegrationTest.userId
          problemId = problem.id
          problemLanguageId = configurationId
          sourceCode = "forged"
          totalCases = 0
  ...
  private fun submitSolutionAsUser(session: MockHttpSession): Result<SubmitSolutionOutcome> {
    ...
    return try {
      runCatching { problemSubmissionService.submitSolution(configurationId, "solution source") }
    } finally {
      SecurityContextHolder.clearContext()
    }
  }

  private fun assertSingleAcceptedProblemSubmission(outcomes: List<Result<SubmitSolutionOutcome>>) {
  ...
  private fun storedProblemSubmissions(): List<ProblemSubmission> = entClient.problemSubmissions.query()
    .all(fixtureContext)
    .getOrThrow()

  private fun createProblemSubmissionDirectly(
    sourceCode: String,
  ): ProblemSubmission =
    entClient.problemSubmissions.create {
      userId = this@ProblemSubmissionIntegrationTest.userId
  ...
  private fun storedCases(): List<ProblemSubmissionFailure> = entClient.problemSubmissionFailures.query()
    .all(fixtureContext)
    .getOrThrow()
    ...
    """
      mutation Submit(${'$'}input: SubmitSolutionInput!) {
        submitSolution(input: ${'$'}input) {
          __typename
          ... on SubmitSolutionSuccess { problemSubmission { $PROBLEM_SUBMISSION_FIELDS } }
          ... on ProblemSubmissionValidationFailure { message fieldErrors { field message } }
  ...
  private fun poll(id: String, session: MockHttpSession? = this.session): JsonNode = graphql(
    "query Poll(${'$'}id: ID!) { problemSubmission(id: ${'$'}id) { $PROBLEM_SUBMISSION_FIELDS } }",
    mapOf("id" to id),
    session,
  )["problemSubmission"]
  ...
  class ProblemSubmissionTestConfiguration {
    @Bean
    @Primary
  ...
  companion object {
    private const val PROBLEM_SUBMISSION_FIELDS = """
