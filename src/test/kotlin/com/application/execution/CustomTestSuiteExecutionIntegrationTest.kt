...
@Import(CustomTestSuiteExecutionIntegrationTest.ExecutionTestConfiguration::class)
class CustomTestSuiteExecutionIntegrationTest {
  @Autowired
  ...
  private lateinit var customScheduler: CustomTestSuiteScheduler
  ...
  fun setUp() {
    ...
    configurationId = entClient.problemLanguages.create {
      problemId = this@CustomTestSuiteExecutionIntegrationTest.problemId
  ...
  fun `custom inputs are prepared asynchronously and every submitted result is retained`() {
    ...
    executor.submitted = { inputs ->
      ...
      assertNull(job.submissionId)
      assertEquals(GradingJobStatus.RUNNING, job.status)
      assertEquals(inputs, cases(job.customTestSuiteRunId!!).map { it.expectedOutputJson })
  ...
  fun `recovery finishes interrupted preparations and preserves ready custom jobs`() {
    ...
    customRunService.claimNextQueuedCustomRun()!!
    val readyId = enqueue(listOf("null", "1"))
    val ready = customRunService.claimNextQueuedCustomRun()!!
    customRunService.saveExpectedOutputsAndEnqueueGradingJob(ready.id, listOf(JsonNull, JsonPrimitive(1)))
    ...
    assertEquals(ready.startedAt, storedRun(ready.id).startedAt)
  ...
  fun `failed Docker recovery leaves running custom jobs untouched until cleanup succeeds`() {
    val id = enqueue()
    val run = customRunService.claimNextQueuedCustomRun()!!
    customRunService.saveExpectedOutputsAndEnqueueGradingJob(run.id, cases(run.id).map { it.inputJson })
  ...
  fun `reference interruption finishes the run and propagates with the interrupt flag restored`() {
    ...
    try {
      assertThrows(InterruptedException::class.java) { customScheduler.prepareQueuedCustomRuns() }
  ...
  fun `a failed job insert rolls back every prepared answer`() {
    ...
    val failingClient = EntClient(PostgresDriver(dataSource, autoDdl = false)) {
      policies {
        customTestSuiteRuns(context.getBean(CustomTestSuiteRunPolicy::class.java))
    ...
    val failingService = CustomTestSuiteRunService(
      failingClient,
    ...
    assertTrue(storedJobs().isEmpty())
    assertEquals(CustomTestSuiteRunStatus.RUNNING, storedRun(run.id).status)
    assertNull(storedRun(run.id).caseResults)
  ...
  fun `official and custom jobs share the queue without repeating reference preparation`() {
    val official = entClient.submissions.create {
      userId = this@CustomTestSuiteExecutionIntegrationTest.userId
      problemId = this@CustomTestSuiteExecutionIntegrationTest.problemId
      problemLanguageId = configurationId
    ...
    assertEquals(SubmissionVerdict.ACCEPTED, entClient.submissions.findById(fixtures, official.id).getOrThrow()!!.verdict)
    assertEquals("RUNNING", poll(id)["status"].asString())
  ...
  fun `grading progresses while reference preparation is still running`() {
    ...
    val readyId = enqueue()
    val ready = customRunService.claimNextQueuedCustomRun()!!
    customRunService.saveExpectedOutputsAndEnqueueGradingJob(ready.id, cases(ready.id).map { it.inputJson })
    ...
    Executors.newSingleThreadExecutor().use { worker ->
      val preparation = worker.submit { customScheduler.prepareQueuedCustomRuns() }
      try {
  ...
  private fun enqueue(inputs: List<String> = listOf("1", "2", "3"), source: String = "user source"): String {
    val response = graphql(
      """mutation(${'$'}input: EnqueueCustomTestSuiteRunInput!) {
        enqueueCustomTestSuiteRun(input: ${'$'}input) {
          __typename ... on EnqueueCustomTestSuiteRunSuccess { customTestSuiteRunId }
        }
      }""".trimIndent(),
    ...
    assertEquals("EnqueueCustomTestSuiteRunSuccess", response["__typename"].asString(), response.toString())
    return response["customTestSuiteRunId"].asString()
  }

  private fun poll(id: String, viewer: MockHttpSession? = session): JsonNode = graphql(
    """query(${'$'}id: ID!) { customTestSuiteRun(id: ${'$'}id) {
  ...
  private fun configureSchedulers(executor: CodeExecutionService) {
    ...
    customScheduler = CustomTestSuiteScheduler(customRunService, settingsService, executor, startup)
  }
  ...
  private fun cases(runId: Long): List<CustomTestCase> =
  ...
  private fun storedRun(runId: Long): CustomTestSuiteRun =
    entClient.customTestSuiteRuns.findById(fixtures, runId).getOrThrow()!!
